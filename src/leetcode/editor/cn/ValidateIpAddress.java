...
public class ValidateIpAddress {
    ...
    class Solution {
        String UNVALUED = "Neither";
        String IPV4 = "IPv4";
        String IPV6 = "IPv6";

        public String validIPAddress(String IP) {
            if (IP.contains(".")) {
                if (IP.startsWith(".") || IP.endsWith(".")) return UNVALUED;
                String[] strings = IP.split("\\.");
                if (strings.length != 4) return UNVALUED;
                for (String str : strings) {
                    if (!str.equals("0") && str.startsWith("0")) return UNVALUED;
                    try {
                        int n = Integer.parseInt(str);
                        if (n < 0 || n > 255) return UNVALUED;
                ...
                if (IP.startsWith(":") || IP.endsWith(":")) return UNVALUED;
                String[] strings = IP.split(":");
                if (strings.length != 8) return UNVALUED;
                for (String s : strings) {
                    if (s.length() > 4 || s.length() == 0) return UNVALUED;
